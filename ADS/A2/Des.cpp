...
void keyOperations()
{	
	cout<<"bool:"<<specialKeyStates[GLUT_KEY_DOWN]<<"   count:"<<choice<<endl;
	if(specialKeyStates[GLUT_KEY_UP]==true && choice<7)
	{
		choice=choice+1;
	}
	else if(specialKeyStates[GLUT_KEY_DOWN]==true && choice>1)
	{
		choice=choice-1;
	}	
}
	...
	while(max_count>=count)
	{
		...
		while(H1->size>0 && (H1->P[0]->key)/1000==count)
		{
			...
			else
			{
				...
				while(nd1!=NULL)
				{
					nd1->pressure=(nd1->pressure)/5+nd2->pressure+1;//nd1->pressure+
...
{
	char ch;
	cout<<"Enter main path number of queues: ";
	cin>>n1;
	cout<<"Enter 2nd path number of queues: ";
	cin>>n2;
