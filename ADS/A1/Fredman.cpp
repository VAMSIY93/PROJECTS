...
long double emin_time=0.0,radix_time=0.0,fred_time=0.0;
long sec,usec;
struct timeval help1,help2;
    ...
    if(temp!=NULL)
    {
        if(temp->child!=NULL)
        {
            ...
            if(H->size==1)
            {
                ...
                H->head=NULL;
                H->size=0;
                gettimeofday(&help2,NULL);
                sec=(help2.tv_sec)-(help1.tv_sec);
                usec=(help2.tv_usec)-(help1.tv_usec);
                long double dif=(sec*1000 + usec/1000.0);
                emin_time=emin_time+dif;
                return temp;
            }   
            temp=H->min;
            H->min=H->min->right;
...
Graph* ReadGraphFromFile(int* E)
{
    ...
    string s;
    fstream f1;
    f1.open("inputGraph-4.txt",ios::in);
    while(getline(f1,s))
    {
      if(s[0]=='#')
          break;
...
int main() 
{
    ...
    cout<<"EMIN:"<<emin_time<<endl;
    cout<<"FREDMAN:"<<fred_time<<endl;
    cout<<"RADIX:"<<radix_time<<endl;
