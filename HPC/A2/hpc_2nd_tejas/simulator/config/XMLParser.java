...
public class XMLParser 
{
	...
	private static void setSystemParameters()
	{
		...
		for (int i = 0; i < SystemConfig.NoOfCores; i++)
		{
			...
			//set Load Store Predictor Parameters
			core.lsPredictor = new LSPredictorConfig();
			Element lspredictorElmnt = (Element)(coreElmnt.getElementsByTagName("LSPredictor").item(0));
			setLSPredictorProperties(lspredictorElmnt, core.lsPredictor);
			core.LSMispredPenalty = Integer.parseInt(getImmediateString("LSMispredPenalty",lspredictorElmnt));
	...
	private static void setLSPredictorProperties(Element predictorElmnt, LSPredictorConfig lsPredictor)
	{
		String tmpStr = getImmediateString("Predictor_Mode", predictorElmnt);
		if (tmpStr.equalsIgnoreCase("AnyStore"))
			lsPredictor.predictorMode = LSP.AnyStore;
		else if(tmpStr.equalsIgnoreCase("LoadStorePair"))
			lsPredictor.predictorMode = LSP.LoadStorePair;
		else if(tmpStr.equalsIgnoreCase("StoreSet"))
			lsPredictor.predictorMode = LSP.StoreSet;
		
		lsPredictor.PCBits = Integer.parseInt(getImmediateString("PCBits",predictorElmnt));
	}
