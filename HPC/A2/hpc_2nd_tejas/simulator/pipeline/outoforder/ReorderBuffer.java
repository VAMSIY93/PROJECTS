...
import pipeline.lspredictor.LSPredictor;

import memorysystem.LSQEntry;
import config.CoreConfig;
import config.LSPredictorConfig;
import config.LSPredictorConfig.LSP;

public class ReorderBuffer extends SimulationElement{

	public long TotalLoadActuallyTrue = 0;
	public long TotalLoadPredictedTrue = 0;
	public long TotalMemoryInstruction = 0;
	...
	{	
		...
		boolean anyMispredictedBranch = false, anyMispredictedLoadStore = false;
...
/*				
				if(firstOpType == OperationType.load)
				{
					boolean predicted = first.getLsqEntry().getPredicted();
					boolean outcome = first.getLsqEntry().getOutcome();
					//if(containingMemSys.core.coreConfig.lsPredictor.predictorMode == LSP.AnyStore)
						execEngine.getLSPredictor().Train(lastValidIPSeen,predicted,outcome,1,false);
					if(predicted!=outcome)
					{
						anyMispredictedLoadStore = true;
						lsmispredCount++;
					}
...
*/
				...
				if(firstOpType == OperationType.load || firstOpType == OperationType.store)
				{
					...
					if (firstOpType == OperationType.store)
					{
						int tmpIndex;
						int ctr = 2;
					
						LSQEntry entry = first.getLsqEntry();
						
						int index = entry.getIndexInQ();
						
						if (entry.getIndexInQ() != execEngine.getCoreMemorySystem().getLsqueue().head)
						{
							tmpIndex = execEngine.getCoreMemorySystem().getLsqueue().decrementQ(index);

						    while(true)
						    {
							    if(ctr > execEngine.getCoreMemorySystem().getLsqueue().curSize)
							    {
								    break;
							    }
							    ctr++;
							
							    LSQEntry tmpEntry = execEngine.getCoreMemorySystem().getLsqueue().lsqueue[tmpIndex];
								if (tmpEntry.getType() == LSQEntry.LSQEntryType.LOAD)
								{
									if (tmpEntry.isValid())
									{
										...
										{
											if (tmpEntry.isIssued() && tmpEntry.getRobEntry().getInstruction()!=null)
											{
												anyMispredictedLoadStore=true;
												lsmispredCount++;
												// train pairLS
												if (core.coreConfig.lsPredictor.predictorMode == LSP.StoreSet)
												{
													execEngine.getLSPredictor().Train(first.getInstruction().getCISCProgramCounter(),false,false,tmpEntry.getRobEntry().getInstruction().getCISCProgramCounter(),anyMispredictedLoadStore);
												}
											}
											else
												TotalLoadPredictedTrue++;
										}
									}				
									else
									{					
								...
								tmpIndex = execEngine.getCoreMemorySystem().getLsqueue().decrementQ(tmpIndex);
							}
						}	
					}
			/*		
					if (!anyMispredictedLoadStore)
					{
						if (core.coreConfig.lsPredictor.predictorMode == LSP.StoreSet)
							execEngine.getLSPredictor().incrementAccuracy();
					}
			*/		
				if(firstOpType == OperationType.load)
				{
					boolean predicted = first.getLsqEntry().getPredicted();
					boolean outcome = first.getLsqEntry().getOutcome();
					if(core.coreConfig.lsPredictor.predictorMode == LSP.AnyStore)
						execEngine.getLSPredictor().Train(lastValidIPSeen,predicted,outcome,1,false);
					if(predicted!=outcome)
					{
						anyMispredictedLoadStore = true;
...
//								+ " : " + first.getPhysicalDestinationRegister());
	...
	void handleLSMisprediction()
	{
		if(SimulationConfig.debugMode)
			System.out.println("Load Store Misprediction");

		execEngine.setToStall5(true);

		core.getEventQueue().addEvent(new MispredictionPenaltyCompleteEvent(GlobalClock.getCurrentTime() + core.getLSMispredictionPenalty()*core.getStepSize(), null, this, RequestType.MISPRED_PENALTY_COMPLETE));
	}
