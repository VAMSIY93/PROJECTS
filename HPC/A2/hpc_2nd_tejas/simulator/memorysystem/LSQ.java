...
public class LSQ extends SimulationElement
{
	...
	public LSQEntry[] lsqueue;
	protected int tail; // points to last valid entry
	public int head; // points to first valid entry
	public int lsqSize;
	public int curSize;
	...
	public LSQSearchStatus setPredOutcome(LSQEntry entry,LSQSearchStatus couldForward)
	{
		...
		ReorderBufferEntry first = entry.getRobEntry();
		Instruction firstInstruction = first.getInstruction();
		lastIP = firstInstruction.getCISCProgramCounter();
		long prediction = containingMemSys.core.getExecEngine().getLSPredictor().predict(lastIP,1);


		if(prediction==1)
		{
			entry.setPredicted(true);
			return LSQSearchStatus.Forwarded;			
		}
		else
		{
			entry.setPredicted(false);
	...
	public boolean loadValidate(LSQEntry entry)
	{
		...
		if(containingMemSys.core.coreConfig.lsPredictor.predictorMode == LSP.AnyStore)
			forwardResult = this.setPredOutcome(entry,couldForward);
		//System.out.println("forw :"+forwardResult +"\tac:"+couldForward);	// Doubt HERE????????????
		if(couldForward == LSQSearchStatus.Forwarded) 
		{
			return true;
		}
	...
	protected LSQSearchStatus loadResolve(int index, LSQEntry entry)
	{
		...
		while(true)
		{
			tmpEntry = lsqueue[tmpIndex];
			if (tmpEntry.getType() == LSQEntry.LSQEntryType.STORE) //&& containingMemSys.core.coreConfig.lsPredictor.predictorMode != LSP.StoreSet)
			{
		...
		if (tmpEntry.getType() == LSQEntry.LSQEntryType.STORE)// && containingMemSys.core.coreConfig.lsPredictor.predictorMode == LSP.StoreSet)
		{
			long a1=entry.getRobEntry().getInstruction().getCISCProgramCounter();
			long a3=tmpEntry.getAddr();
			//System.out.println(a1+"  "+a3+"   "+containingMemSys.core.getExecEngine().getLSPredictor().predict(a1,a3));
			long predictedStorePC;
			if(containingMemSys.core.getExecEngine().getLSPredictor().predict(a1,a3) == 1)
			{
				//System.out.println("silen kiilere");
				entry.setPredicted(true);
				entry.setForwarded(true);
				entry.setOutcome(true);
				NoOfForwards++;
			...
			else if ((predictedStorePC = containingMemSys.core.getExecEngine().getLSPredictor().predict(a1,a3)) > 0)
			{
				//System.out.println("Come regularly.");
				entry.setPredicted(true);
				entry.setForwarded(true);
				entry.setOutcome(true);
				NoOfForwards++;
	...
	public int incrementQ(int value)
	{
		value = (value+1)%lsqSize;
		return value;
	}
	public int decrementQ(int value)
	{
		if (value > 0)
			value--;
		else if (value == 0)
