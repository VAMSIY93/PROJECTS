...
func (node Node) insertToList(contents []string) Node {
	...
	if node.nodeId==2 {
	fmt.Println("c3: ",contents[3])
	for i:=0;i<length;i++ {
		final = final + node.txn_list[i] + ":"
	}
	fmt.Println("final1: ",final)
	}
